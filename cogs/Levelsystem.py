...
class LevelSystem(commands.Cog):
    ...
    async def levels(self, ctx, choice = None):
        ...
        if choice == None:
            await ctx.message.reply("Invalid command usage, please use it like this: `ms!levels enable/disable`")
            return
        if choice.lower() == "enable":
            ...
            await ctx.message.reply("Levelling system has now been **enabled** for this server. To configure level up message channel please use `ms!levelupchannel <#channel>`.")
        elif choice.lower() == "disable":
            if hmph == None:
                await ctx.message.reply("Levelling system is already **disabled** for this server. To enabled it please use `ms!levels enable`")
                return
    ...
    async def level_up_channel(self, ctx, channel: discord.TextChannel = None):
        ...
        if hmmm_yes == None:
            await ctx.message.reply("Levels have not been enabled for this server yet. Please use `ms!levels enable`.")
            return
